...
**Priority tier:** **3 — on-site in Bangladesh** (Dhaka, full-time permanent, hybrid). The fallback
tier. Applied to before the tier ranking existed; see `CLAUDE.md` § Job-search preferences.
...
| 2026-07-25 | `Applied` | **Submitted via the `recruitment.welldev.io` portal.** Recorded retroactively on 2026-08-03 — the earlier `Draft` rows were never updated at the time. Date taken as the posting's deadline day; the true submission date is on or before 2026-07-25. |
| ~2026-07-31 | `Task/Test` | **MCQ test sat**, per the JD's tentative date. **Awaiting the result.** |
| 2026-08-03 | `Task/Test` | Workspace restructured: `jobs/` split into `Findings/` and `Applied/`, log moved to `jobs/jobs_log.csv`. This folder moved to `jobs/Applied/`; no file inside it was altered except this timeline and the priority-tier header. |
